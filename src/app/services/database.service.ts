...
export class DatabaseService {
  ...
  async addUser(user: User): Promise<boolean> {
    try {
      // Simula agregar un usuario a la base de datos
      // Aquí debes implementar la lógica para interactuar con la base de datos real
      console.log('Usuario registrado:', user);
      return true; // Devuelve true si el usuario se registró correctamente
    } catch (error) {
      showAlertError('DatabaseService.addUser', error);
      return false;
    }
  }
  ...
  async findUserByEmail(email: string): Promise<User | undefined> {
    try {
      ...
      console.log(`Resultado de la consulta findUserByEmail para el email ${email}:`, rows);
      return rows?.length ? this.rowToUser(rows[0]) : undefined;
    } catch (error) {
      showAlertError('DataBaseService.findUserByEmail', error);
      return undefined;
  ...
  async getAllUsers(): Promise<User[]> {
    try {
      // Llama a readUsers para obtener todos los usuarios
  ...
  private rowToUser(row: any): User {
    try {
      ...
      user.dateOfBirth = convertStringToDate(row.dateOfBirth) || new Date();
      user.address = row.address;
      return user;
    } catch (error) {
      showAlertError('DataBaseService.rowToUser', error);
      console.error('Error en rowToUser:', error);
    }
