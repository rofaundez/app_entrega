...
export class AuthService {
  ...
  async register(user: User): Promise<boolean> {
    try {
      ...
      const isRegistered = await this.db.addUser(user);
  
      if (isRegistered) {
        showToast(`¡Registro exitoso! Bienvenid@ ${user.firstName} ${user.lastName}`);
        await this.saveAuthUser(user); // Guarda al usuario autenticado
        await this.router.navigate(['/inicio']); // Navega al inicio
        return true;
      } else {
        showToast('No se pudo registrar el usuario. Inténtalo de nuevo.');
        return false;
      }
    } catch (error) {
